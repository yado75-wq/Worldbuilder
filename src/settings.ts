...
export class WorldBuilderSettingTab extends PluginSettingTab {
	...
	getSettingDefinitions() {
		return [
			...
			{
				name: 'Template sets',
				// no control / render → just a heading
			},

			// ── Dynamic list of template sets ─────────────────────────────
			{
				name: 'Template sets list',
				render: (setting: Setting) => {
					// We hide the default name/desc of this row and build everything ourselves
					...
					for (const set of templateSets) {
						...
						if (!set.isValid) {
							row.nameEl.addClass('wb-invalid');
						}
					}
				},
			},
			...
			{
				name: this.plugin.state.activeWorld
					? this.plugin.state.activeWorld.name
					: 'No active world',
				desc: this.plugin.state.activeWorld
					? `Template set: ${this.plugin.state.activeWorld.templateSet}`
